...
from Powerups import LongerStaff_P, DoubleScore_P, Peach_P
...
# Double Score
game_started_time_double_score = pygame.time.get_ticks() # First 4 second check
	...
	def powerup_update(self):
		...
		if self.peach_status == True:	
			...
			if (pygame.time.get_ticks() - peach_picked_up_time) < peach_status_duration: 
				...
				draw_text(': ' + str(round(10 - (pygame.time.get_ticks() - peach_picked_up_time) / 1000,2))   , score_font, WHITE, screen_width - 230, screen_height - 100)
				# Draw the peach status icon  
				screen.blit(peach_status_icon_image,(screen_width - 270, screen_height - 108))
		...
		if self.double_score_status == True: 
			#For the duration of the effect wears off
			if (pygame.time.get_ticks() - double_score_powerup_picked_up_time) < double_score_status_duration:
				# Draw the double score status text and icon
				draw_text(': ' + str(round(10 - (pygame.time.get_ticks() - double_score_powerup_picked_up_time) / 1000,2))   , score_font, WHITE, screen_width - 330, screen_height - 100)
				screen.blit(double_score_status_icon_image,(screen_width - 370, screen_height - 108))
			else:
	...
	def powerup_update(self, x, y):
		...
		if staff_count > 0:
			...
			if self.longer_staff_status == True:
				...
				draw_text(': Enabled ', score_font, GREEN, screen_width - 480, screen_height - 100)
				# Draw the longer staff status icon  
				screen.blit(longer_staff_status_icon_image,(screen_width - 520, screen_height - 108))
...
ground = Ground(0, screen_height - 130)
...
while run:
	...
	# Draw score text
	draw_text('Score: ' + str(total_score), score_font, WHITE, screen_width - 150, screen_height - 100)
	...
	if pygame.time.get_ticks() - game_started_time_double_score > collectible_powerups_spawning_cooldown:
		...
		if num_of_collectible_powerups < 2:
			...
			# Pick between two numbers, one of them will spawn the double score_powerup, 
			choose_powerup_number = random.choice([1, 2])
			...
			game_started_time_double_score = pygame.time.get_ticks()
	...
	if pygame.sprite.spritecollide(player, longer_staff_powerup_group, False):
		if pygame.sprite.spritecollide(player, longer_staff_powerup_group, True, pygame.sprite.collide_mask):
			# Lower then number of collectibles in the map currently
			num_of_collectible_powerups -= 1
			# I want to permanently modify the length of the staff
	...
	if staff_count > 0:
		...
		# Check for powerups and activate their effects if the conditions are met
		#staff.powerup_update()
