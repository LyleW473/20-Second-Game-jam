...
clicked = False
click_cooldown = 1000 # 1.75 seconds
staff_count = 0
...
class Wukong_Staff(pygame.sprite.Sprite):
	...
	def rotate(self):
		...
		#if self.position_finding == False:
...
while run:	
	...
	if clicked == True:
		...
		if num_of_throws == 0:
			pass
		elif num_of_throws > 0:
			if len(time_now_list) == 0: 
				time_now = pygame.time.get_ticks()
				time_now_list.append(time_now)
				print("time_now_list", time_now_list)

			if time_now_list[0] - on_ground_time_list[0] > click_cooldown:
				staff.position_finding = True
				print("time_now_list[0] - on_ground_time_list[0] = ",time_now_list[0] - on_ground_time_list[0])


			#print("2nd and onwards click",time_now)
			#if time_now - staff.initial_click_time


		# If there are no staffs then instantiate a staff
		if staff_count == 0:
			if num_of_throws == 0:
				...
				#staff.clicked = True
				print("instantiated")
				staff = Wukong_Staff(player.rect.centerx + initial_staff_position_spacing , 600 - special_initial_staff_position_spacing) # staff = Wukong_Staff(x , 777)


			# Start finding the clicked position
			staff.position_finding = True
