...
from Powerups import LongerStaff_P, DoubleScore_P, Peach_P, FasterMovementSpeed_P
...
game_started_time_double_score = pygame.time.get_ticks() # First 4 second check, this is up here because it applies to all collectible powerups that are spawned on the tiles
...
faster_movement_speed_status_duration = 10000
...
class Player():
	...
	def powerup_update(self):
		...
		screen.blit(faster_movement_speed_status_icon_image,(screen_width - 850 , screen_height - 78))
		...
		if self.faster_movement_speed_status == True:
			...
			if (pygame.time.get_ticks() - faster_movement_speed_powerup_picked_up_time) < faster_movement_speed_status_duration:
				...
				draw_text(': ' + str(round(10 - (pygame.time.get_ticks() - faster_movement_speed_powerup_picked_up_time) / 1000,2))   , score_font, WHITE, screen_width - 810, screen_height - 70)	
...
class Wukong_Staff(pygame.sprite.Sprite):
	...
	def powerup_update(self, x, y):
		...
		# If the powerup hasn't been activated yet, just draw the icon.
		if self.longer_staff_status == False: 
...
while run:
	...
	if (pygame.time.get_ticks() - game_started_time_double_score) > collectible_powerups_spawning_cooldown:
		...
		if num_of_collectible_powerups != 2:
			...
			choose_powerup_number = random.randint(1,3)
	...
	if pygame.sprite.spritecollide(player, double_score_powerup_group, False):
		if pygame.sprite.spritecollide(player, double_score_powerup_group, True, pygame.sprite.collide_mask):
			...
			# Reset the game_started_time_double_score to be the same as the current time so that it will count 4 seconds from when the player picked up the powerup
			game_started_time_double_score = pygame.time.get_ticks()
	...
	# REMEMBER TO MOVE THESE DOWN LTER
