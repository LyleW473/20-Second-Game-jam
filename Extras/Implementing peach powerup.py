...
# Powerups variables
game_started_time = pygame.time.get_ticks() # Use this to find out when the game started (later on we'll add a countdown), and this will be used as a check for multiple stuff
	...
	def powerup_update(self):
		if self.peach_status == True:	
			...
			if (pygame.time.get_ticks() - peach_picked_up_time) < peach_status_duration: 
				...
				draw_text(': ' + str(float(pygame.time.get_ticks() - peach_picked_up_time) / 1000), score_font, WHITE, screen_width - 230, screen_height - 100)
...
staff_e_powerup_group = pygame.sprite.Group()
score_m_powerup_group = pygame.sprite.Group()
...
while run:
		...
		if staff_count > 0:
			...
			if pygame.sprite.spritecollide(staff, enemy_group, False):
				...
				if pygame.sprite.spritecollide(staff, enemy_group, True, pygame.sprite.collide_mask):
					...
					# Increment combo
					total_score += staff.score_added 
					# Draw the score added onto the screen
					combo_text = TextEffect(staff.rect.centerx, staff.rect.centery,'+' + str(staff.score_added), WHITE)
	...
	# Peach powerup
	# Note: Every 4 seconds, a peach powerup is spawned 



	# Check if enough time has passed since the start of the game
	if (pygame.time.get_ticks() - game_started_time) > peach_spawning_cooldown:
		print(pygame.time.get_ticks() - game_started_time)
	...
	if pygame.sprite.spritecollide(player, peach_powerup_group, False):
		...
		if pygame.sprite.spritecollide(player, peach_powerup_group, True, pygame.sprite.collide_mask):
			...
			total_score += (score_add * 10) # Adds 50 score to the player's total score
			# Draw the text that displays how much was added to the score
			peach_text = TextEffect(player.rect.centerx, player.rect.centery,'+' + str(score_add * 10), GOLD)
	...
	# Check for if any powerups have been picked up
	player.powerup_update()

	# Update the peach powerups
