...
def reset_level(staff_count, game_started, allowed_to_click, amount_of_time, total_score, game_started_time_peach, game_started_time_collectible_powerups, num_of_collectible_powerups, random_index_list, starting_enemy_spawned):
	# Staff 
	...
	return staff_count, game_started, allowed_to_click, amount_of_time, total_score, game_started_time_peach, game_started_time_collectible_powerups, num_of_collectible_powerups, random_index_list, starting_enemy_spawned
...
game_started = False # Variable indicates when we are in game
...
#--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		...
		if amount_of_time > 0 and menu.show_paused_menu == False:	
			...
			draw_alpha_text(str(round(amount_of_time / 1000 , 2)), countdown_font, countdown_colour, (screen_width / 2) - 140, (screen_height / 2) - 450)
			...
			# cursor_position = pygame.mouse.get_pos() Note: Moved this further up to avoid errors 
...
#--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		# If the 20 second countdown has ended then do this:
			...
			# The game is no longer still active so set this variable to False
			game_started = False
			...
			if menu.reset_level == True:
				...
				staff_count, game_started, allowed_to_click, amount_of_time, total_score, game_started_time_peach, game_started_time_collectible_powerups, num_of_collectible_powerups, random_index_list, starting_enemy_spawned = reset_level(staff_count, game_started, allowed_to_click, amount_of_time, total_score, game_started_time_peach, game_started_time_collectible_powerups, num_of_collectible_powerups, random_index_list, starting_enemy_spawned)
	...
	for event in pygame.event.get():
		...
		if event.type == pygame.MOUSEBUTTONDOWN and allowed_to_click == True and game_started == True:
			# Note: game_started variable is so that when clicking the Play button, the staff doesn't get thrown out yet
